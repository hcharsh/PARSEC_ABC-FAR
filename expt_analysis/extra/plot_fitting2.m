function plot_fitting2(info, nest_ind)
warning off;
%% figure formatting
...
%% information
img_file_name = [info.system_file, '_s'];
...
nest_being_used = min([nest_ind, n_nest]);
PRM_last = PRM_nest(:, :, nest_being_used);
folder = ['RES_nest_', num2str(nest_being_used)]; mkdir(folder);
...
% legend boxoff;
print(f, [folder, '/', img_file_name] ,'-djpeg', '-r960');
print(f, [folder, '/', img_file_name] ,'-depsc');
% savefig(f,[info.mainfolder, '/', img_file_name, '.fig']);

print(f, ['all_RES/', img_file_name, '_', num2str(nest_being_used)] ,'-djpeg', '-r960');
